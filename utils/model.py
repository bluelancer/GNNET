...
def sparse_identity(size):
    i = tf.arange(size)
    diag = tf.stack([i, i], axis=1)
    return tf.sparse.SparseTensor(
        diag, tf.ones(size, dtype=tf.float32), (size, size))


class SurgeonModule(tf.keras.Model):
    @property
    def device(self):
...
class AugmentationLayer(SurgeonModule):
    ...
    def __init__(self, in_dim, out_dim, dropout):
        ...
        self.aug1 = tf.keras.Sequential([tf.keras.layers.InputLayer(input_shape=in_dim),
										 tf.keras.layers.Dense(out_dim, activation=None)])
        self.aug2 = tf.keras.Sequential([tf.keras.layers.InputLayer(input_shape=in_dim),
										 tf.keras.layers.Dense(out_dim, activation=None)])
        
    def forward(self, x):
        x1 = self.aug1(x)
        x1 = tf.keras.layers.Dropout(x1, self.dropout)
        x2 = self.aug2(x)
        x2 = tf.keras.layers.Dropout(x1, self.dropout)
        return x1, x2
