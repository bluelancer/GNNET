...
from utils.datanetAPI import DatanetAPI, DatanetException
...
class PreprocessedGenerator:
	def __init__(self, data_dir=None, shuffle=False, complete_info=False, filter_size = False, filter_operator = None, debug = False, interval_size =False, interval = [None,None]):
		...
		if self.interval_size:
			self.lower_interval = interval[0]
			self.higher_interval =  interval[1]
		self.debug = debug
	...
	def get_available_sample_files(self):
				...
				if filename.endswith(extension):
					if self.filter_size:
						...
						if self.debug:
							...
							print('root.decode(utf-8))[1]',os.path.split(root.decode('utf-8'))[1])
						if eval(os.path.split(root.decode('utf-8'))[1] + self.filter_operator.decode()):
							self._file_paths.append(os.path.join(root, filename))
							# print ('file_path',os.path.join(root, filename))
					if self.interval_size:
						if (
							(int(os.path.split(root)[1]) 
								> self.lower_interval -1) 
							and 
							(int(os.path.split(root)[1])
								<  self.higher_interval+1)):
							self._file_paths.append(os.path.join(root, filename))
					if not self.filter_size and not self.interval_size:
		...
		if self._shuffle:
			...
			self._file_paths = np.sort(self._file_paths)
		# print ('self._file_paths',self._file_paths)
...
def input_fn(data_dir, shuffle=False, samples=None, complete_info=False, pre_processed=True, filter_size = False, filter_operator = "False", debug = False,interval_size =False, interval = [0,0]):
	...
	ds = tf.data.Dataset.from_generator((lambda *x: PreprocessedGenerator(*x)()) if pre_processed else generator,
										args=[data_dir, shuffle,complete_info,
											  filter_size,filter_operator,debug,interval_size,interval],
