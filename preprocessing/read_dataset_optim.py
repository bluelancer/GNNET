...
class PreprocessedGenerator:
	def __init__(self, data_dir=None, shuffle=False, complete_info=False):
...
def generator(data_dir=None, shuffle=False, complete_info=False, datanet_obj=None):
	...
	num_samples = 0
	for sample in it:
		global_packets = sample.global_packets
		...
		# adding global packet
		input_data[0]['global_packet'] = global_packets
        ...
        for dst in range(G.number_of_nodes()):
            if (src != dst):
                if len(R[src, dst]) == 2 and len(T[src, dst]['Flows']) != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                    ...
                    for h_1, h_2 in [R[src, dst][i:i + 2] for i in range(0, len(R[src, dst]) - 1)]: 
                        D_G.add_edge('l_{}_{}'.format(h_1, h_2),'l_{}_{}'.format(h_1, h_2), type = 1, weight =(T[h_1,h_2]['AggInfo']['AvgBw']/traffic_agg_by_hop[h_1][h_2]))
                ...
                if len(R[src, dst]) > 2:
                    ...
                    for h_1, h_2, h_3 in [R[src, dst][i:i + 3] for i in range(0, len(R[src, dst]) - 2)]:  
                        ...
                        if len(T[src, dst]['Flows']) != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                            ...
                            mutual_information = common_traffic/fore_traffic
...
def input_fn(data_dir, shuffle=False, samples=None, complete_info=True, pre_processed=True):
