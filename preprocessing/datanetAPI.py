...
class DatanetAPI:
    ...
    def set_files_to_process(self, tuple_files_lst):
        ...
        if type(tuple_files_lst[0]) is not tuple and len(tuple_files_lst[0]) == 2:
            tuple_files_lst = [tuple(t) for t in tuple_files_lst]
        for file_tuple in tuple_files_lst:
            if not type(file_tuple) is tuple or len(file_tuple) != 2:
                raise DatanetException("ERROR: The argument of set_files_to_process should be a list of tuples -> [(root_dir,file),...]")
            if (not file_tuple in self._all_tuple_files):
